...
const ConversationBlock = ({ userData, }) => {
    ...
    const [isMessageSending_Loading, setisMessageSending_Loading] = useState(false)
    ...
    useEffect(() => {
        const handleKeyPress = (e) => {
            if (e.key === "Enter" && userInput?.trim()?.length > 0 && !isMessageSending_Loading) {
                handleSendMessage();
            }
        };
            ...
            <div className="bg-gray-800 rounded-2xl border border-gray-700 h-[700px] flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto p-6  space-y-6">
                    ...
                    {allMessages.length === 0 ? (
                        ...
                        allMessages?.map((message) => (
                            <div key={message.id} className={`flex gap-4 ${message?.role == "assistant" ? '' : 'justify-end'}`}>
                                ...
                                {/* MESSAGE BUBBLE */}
                                <div className={`rounded-2xl p-6 border max-w-3xl ${message?.role == "assistant"
                ...
                <div className="p-6 border-t border-gray-700">
                    <div className="flex gap-4">
                        ...
                        >
                            {isMessageSending_Loading ? <div className='border-l-2 rounded-full border-r-2 border-t-2 h-6 w-6 border-white animate-spin'></div> : "Send"}
                        </button>
                    </div>
                </div>
            </div>
