...
const JoinedUsersPopUp = ({
      ...
      >
        <div className="flex items-center gap-3 mb-4">
          <div className="p-2 bg-red-500/20 rounded-lg">
            <UserRoundX className="text-red-400 text-2xl" />
          </div>
          <div>
            <h3 className="text-xl font-bold text-white">Kick User</h3>
            <p className="text-gray-400 text-sm">
              Are you sure you want to kick  {allUsersData?.find((usr) => usr?._id == kickingUserId)?.name}?
            </p>
          </div>
        </div>

        <div className="bg-gray-800/50 rounded-lg p-4 mb-6">
          <p className="text-gray-300 ">You'll lose access</p>
          
        </div>

        <div className="flex gap-3">
          <button
            onClick={() => setKickingUserId("")}
          ...
          <button
            ...
            className="flex-1 py-3 px-4 rounded-lg bg-gradient-to-r from-red-500 to-red-600 text-white hover:from-red-600 hover:to-red-700 transition-all font-medium"
          >
            Kick
          </button>
        </div>
  ...
  return (
    <>
      {kickingUserId && <SureKickUserModal />}
      {/* modal start ----------------------------- */}
      {/* <motion.div
        initial={{ display: "none" }}
        transition={{ delay: kickingUserId ? 0 : 0 }}
        animate={{ display: kickingUserId ? "flex" : "none" }}
        className="absolute top-0 w-full h-full left-0 z-50  bg-[#0000005a] flex justify-center items-center "
      >
        <motion.div
          initial={{ scale: 0 }}
          transition={{ duration: 0.3 }}
          animate={{ scale: kickingUserId ? 1 : 0 }}
          className="rounded-xl shadow-xl px-10 bg-white py-4"
        >
          <p className="font-bold text-xl">
            Are you sure? you want to kick "
            {allUsersData?.find((usr) => usr?._id == kickingUserId)?.name}"
          </p>
          <div className="flex mt-6 justify-end gap-5 items-center">
            ...
            <button
              ...
              className="px-6 py-1 hover:scale-110 duration-200   rounded-lg text-white border-2 border-purple-500  bg-purple-600"
            >
              Yes
            </button>
          </div>
            ...
            {allUsersData.map((usr, i) => (
              <div
                ...
                className={`bg-gray-800 rounded-lg p-4 border-l-4 ${
                  i % 2 == 0 ? "border-purple-500" : "border-transparent"
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <h3 className="font-semibold text-white text-lg">
                        {usr?.name}{" "}
                        {usr?._id == roomData?.createdBy?._id && (
